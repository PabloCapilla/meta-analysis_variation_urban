...
##
##### Script aim: #####
#' Script for within-year meta-model of CVR in laying date, clutch size and number of fledglings.
#' Model 5 in main text.
#' 
##
##
...
##### data #####
##
data <- readRDS("./data/processed_RDS_data_files/metaanalysis_WITHINYEAR_data.RDS")
...
phylo_cor <- readRDS("./data/processed_RDS_data_files/phylogenetic_correlations_WIHTIN_model.RDS")

##
## data for model
df_lnCVR_wihtin <- data %>% 
  filter(!is.na(lnCVR)) %>% 
  filter(!is.na(lnCVR.sv))


##
##### Meta-analysis lnCVR within seasons - MODEL 5 in main text #####
##
lnCVR_wihtin <- rma.mv(yi = lnCVR, 
                 V = lnCVR.sv, 
                 mods = ~ trait - 1, 
                 ...
                 struct=c("DIAG", "DIAG"), 
                 data=df_lnCVR_wihtin, 
                 method="REML")

summary(lnCVR_wihtin) # results presented in Table 1
